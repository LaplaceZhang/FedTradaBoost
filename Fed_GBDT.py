...
import numpy as np
import pickle # return trained model
...
class TrAdaBoost():
    ...
    def tradaboost(self, trans_S, trans_A, label_S, label_A, test, N):
        ...
        for i in range(N):
            ...
            result_label[:, i], updated_model = self.train_classify(trans_data, trans_label,
                                                test_data, P)
        ...
        return predict, updated_model

    def calculate_P(self, weights, label):
        total = np.sum(weights)
        return np.asarray(weights / total, order='C')

    def train_classify(self, trans_data, trans_label, test_data, P):  # AdaBoost
